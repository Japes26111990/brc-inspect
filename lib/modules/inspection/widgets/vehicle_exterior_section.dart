...
class VehicleExteriorSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final List<ComponentResult> allItems = provider.sections['Vehicle Exterior'] ?? [];
    
    // 🚀 DYNAMIC MULTI-MODE LAYOUT FILTER
    final List<ComponentResult> filteredItems = allItems.where((component) {
      if (provider.activeType == InspectionType.roadworthy) {
        return component.isRoadworthyRelevant; // Only shows critical lights/glass for safety tests
      }
      if (provider.activeType == InspectionType.fleet) {
        // Fleet keeps it simple: check your primary functional lights and wipers
        return component.isRoadworthyRelevant || component.id == 'wipers' || component.id == 'headlights_cond';
      }
      return true; // Full detail mode
    }).toList();

    return _ExteriorListContainer(items: filteredItems, provider: provider);
  }
}

class _ExteriorListContainer extends StatefulWidget {
  final List<ComponentResult> items;
  final ActiveInspectionProvider provider;
  const _ExteriorListContainer({required this.items, required this.provider});

  @override
  State<_ExteriorListContainer> createState() => _ExteriorListContainerState();
}

class _ExteriorListContainerState extends State<_ExteriorListContainer> {
            ...
            children: [
              ...
              if (!comp.isNotApplicable) ...[
                ...comp.photoTargets.map((target) {
                  bool needsPhoto = comp.id == 'windscreen'; // Mandatory safety glass registration check
