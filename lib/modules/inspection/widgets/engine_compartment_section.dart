...
class EngineCompartmentSection extends StatefulWidget {
  final ActiveInspectionProvider provider;
  const EngineCompartmentSection({super.key, required this.provider});

  @override
  State<EngineCompartmentSection> createState() => _EngineCompartmentSectionState();
}

class _EngineCompartmentSectionState extends State<EngineCompartmentSection> {
  ...
  Widget build(BuildContext context) {
    final List<ComponentResult> items = widget.provider.sections['Engine Compartment'] ?? [];
          ...
          child: Column(
            children: [
              Row(
                children: [
                  Expanded(
                    child: Text(comp.title, style: const TextStyle(color: AppColors.textPrimary, fontSize: 13, fontWeight: FontWeight.bold)),
                  ),
                  Row(
              ...
              if (!comp.isNotApplicable) ...[
                ...comp.photoTargets.map((target) {
                  bool needsPhoto = (comp.id == 'oil_cap_dipstick' || comp.id == 'engine_core');
                        ...
                        if (needsPhoto) ...[
                          Row(
                            children: [
                              ...
                              // 🚀 ENFORCED DIRECTIONAL FINE TARGET TEXT LABELS
                              Expanded(
                                child: Text(
                                  target.label, 
                                  style: const TextStyle(color: AppColors.gold, fontSize: 11, fontWeight: FontWeight.w500, fontStyle: FontStyle.italic)
                                ),
                              ),
